...
const MenuPage: React.FC<MenuPageProps> = ({
  ...
  return (
    <div className="w-full flex flex-col items-center bg-white overflow-x-hidden min-h-screen">
      ...
      <div className="max-w-4xl w-full mx-auto px-6 mb-20 animate-fade-up">
        <div className="bg-[#F8F9FA] rounded-[15px] p-2 flex flex-col gap-3 shadow-[0_20px_60px_rgba(0,0,0,0.04)] border border-gray-100">
          ...
          <div className="flex justify-center pt-2">
            <div className="flex bg-gray-200/40 p-1.5 rounded-[15px] gap-1 items-center">
              {(["All", "Veg", "Non-Veg"] as const).map((type) => (
                <button
                  key={type}
                  onClick={() => setFoodTypeFilter(type)}
                  className={`flex items-center justify-center gap-2 px-6 py-2 rounded-[15px] text-[9px] font-black uppercase tracking-widest transition-all duration-300 min-w-[100px] ${
                    foodTypeFilter === type
                ...
                >
                  {type === "Veg" && (
                    <div className="w-1.5 h-1.5 rounded-[15px] bg-emerald-500 shadow-sm shadow-emerald-500/50"></div>
                  )}
                  {type === "Non-Veg" && (
                    <div className="w-1.5 h-1.5 rounded-[15px] bg-red-500 shadow-sm shadow-red-500/50"></div>
                  )}
                  {type.toUpperCase()}
                </button>
              ))}
          ...
          <div className="flex items-center px-2">
            ...
            >
              <button
                ...
                className={`whitespace-nowrap px-6 py-2.5 rounded-[15px] text-[9px] font-black uppercase tracking-widest transition-all ${
                  category === "All"
          ...
          <div className="flex justify-between items-center px-8 py-3 border-t border-gray-100/50">
            <span className="text-[8px] font-black text-gray-300 uppercase tracking-[0.3em]">
              {filtered.length} CULINARY OPTIONS
            </span>
            <button
              onClick={() => setShowOutletToggle(!showOutletToggle)}
              className="text-[8px] font-black text-[#A94442] uppercase tracking-widest hover:underline"
            >
              SWITCH STATION ↑
            </button>
          </div>
      ...
      <div className="max-w-7xl w-full mx-auto px-3 pb-30">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-[500px] bg-gray-50 animate-pulse rounded-[15px] border border-gray-100"
              ></div>
            ))}
          </div>
        ) : (
...
const ProductCard: React.FC<{
  ...
  return (
    ...
    >
      <div className="relative aspect-[5/3] mb-4 overflow-hidden rounded-[15px] bg-gray-50">
        <img
          src={item.imageUrl}
          alt={item.name}
      ...
      <div className="px-8 pb-10 pt-2 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-1">
          <h3 className="text-2xl font-bold text-gray-900 leading-tight group-hover:text-[#C0392B] transition-colors">
            {item.name}
          </h3>
